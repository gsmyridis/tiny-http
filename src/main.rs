...
fn main() {
    ...
    let _server = HttpServer::build()
        .verbose(true)
        .workers(4)
        .route("/", Box::new(route_home))
        .route("/echo", Box::new(route_echo))
        .route("/user-agent", Box::new(route_user_agent))
        .route("/files", Box::new(move |request: &Request<String>| {
            let filename = request.uri().inner.chars().skip(7).collect::<String>();
            let file_content = std::fs::read_to_string(format!("{dir}/{filename}"))
